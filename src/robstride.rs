...
impl RobStrideUtils {
    ...
    pub fn request_dev_id(&mut self) -> CanFrame {
        CanFrame::new(
            self.set_extended_id(0, 0x0000, self.id), &[0]
        ).unwrap()
    }
    ...
    pub fn request_enable(&mut self) -> CanFrame {
        CanFrame::new(
            self.set_extended_id(3, 0x0000, self.id), &[0]
        ).unwrap()
    }

    pub fn request_stop(&mut self, mode: StopMode) -> CanFrame {
        CanFrame::new(
            self.set_extended_id(4, 0x0000, self.id), &[mode as u8]
        ).unwrap()
    }

    pub fn set_zero_point(&mut self) -> CanFrame {
        CanFrame::new(
            self.set_extended_id(6, 0x0000, self.id), &[1]
        ).unwrap()
    }

    pub fn set_dev_id(&mut self, request_id: u8) -> CanFrame {
        CanFrame::new(
            self.set_extended_id(7, (request_id as u16) << 8, self.id), &[1]
        ).unwrap()
    }
